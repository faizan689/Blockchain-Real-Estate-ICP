...
// Get storage instance from the factory instead of importing directly
  ...
  async analyzeUserProfile(userId: number): Promise<UserInvestmentProfile> {
    try {
      ...
      try {
        ...
        console.error('OpenAI API Error in analyzeUserProfile:', apiError);
        
        // For quota exceeded or rate limit errors, return default profile
        if (apiError?.status === 429 || apiError?.code === 'insufficient_quota' || apiError?.type === 'insufficient_quota') {
          console.log('Using default user profile due to API limits');
          return {
            preferredInvestmentRange: { min: 1000, max: 50000 },
            riskTolerance: 'moderate',
            preferredPropertyTypes: ['residential', 'commercial'],
        ...
        // For other errors, also return default profile
        console.log('Using default user profile due to API error');
        return {
          preferredInvestmentRange: { min: 1000, max: 50000 },
          riskTolerance: 'moderate',
          preferredPropertyTypes: ['residential', 'commercial'],
  ...
  async generateRecommendations(userId: number, limit: number = 5): Promise<PropertyRecommendation[]> {
    try {
      ...
      try {
        ...
        console.log('Generating fallback recommendations due to API error');
        return this.generateFallbackRecommendations(userProfile, availableProperties, limit);
      }
    } catch (error) {
